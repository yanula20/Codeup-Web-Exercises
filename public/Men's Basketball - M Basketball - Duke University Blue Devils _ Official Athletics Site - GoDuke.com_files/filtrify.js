...
;(function ( $, window, document, undefined ) {
	...
	Filtrify.prototype.build = function ( f ) {
		...
		if (tags.length <= 0) {
			this._menu.list.hide();
		}
		
		html = "<li class='ft-field'>" + 
		"<span class='ft-label'>" + f.toUpperCase() + "<i class='icon' /></span>" + 
		"<div class='ft-panel ft-hidden'>";
		
		if (this.options.search) {
			html +=  "<ul class='ft-selected' style='display:none'></ul>" + 
					 "<fieldset class='ft-search'><input type='text' placeholder='Search' /></fieldset>";
		}
		else {
			var ins = 'Click below to filter calendars';
			html +=  "<div class='instructions'>" + ins +"</div><ul class='ft-selected' style='padding-bottom: 10px;'></ul>";
		}
	...
	Filtrify.prototype.filter = function () {
		...
		for ( r = this._matrix.length - 1; r >= 0; r-- ) {
			...
			for ( f in this._query ) {
				...
				var condition = (c >= this._query[f].length);
				if (this.options.filter == 'OR') {
					
					condition = c > 0;
				}
				
				if ( !this._query[f].length  || condition ) {
					// match!
				} else { 
					m = false; 
				};
